...
export function getSavedSheetId(db?: Database): string | null {
  if (db?.settings?.googleSheetId) {
    return db.settings.googleSheetId;
  }
...
export function buildSheetsData(db: Database) {
  ...
  return [
    { range: "'Scoreboard'!A1", values: scoreboardRows },
    { range: "'Winners List'!A1", values: winnersListRows },
    { range: "'Programs List'!A1", values: programsListRows },
    { range: "'Program Results'!A1", values: programOverviewRows },
    { range: "'Participants'!A1", values: participantRows },
    { range: "'System Backup'!A1", values: systemBackupRows },
  ];
}
